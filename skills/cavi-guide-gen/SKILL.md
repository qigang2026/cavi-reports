...
> 不同问题看不同文件。**模板管"字段长啥样"，`MD-生成-总览.md` 管"字段怎么取/怎么用/怎么验"。**

| 你要问的问题 | 权威定义在哪 | 节号 |
|------------|------------|------|
| **字段长啥样**（Key / 类型 / 示例值） | `assets/standard-template-v3.md` | 各段"变量说明"表 |
| **字段从哪取**（系统 / 混合 / AI 可编辑性 + 兜底默认值） | `docs/MD-生成-总览.md` | §2 字段表（权威） |
| **数据怎么获取**（4 数据源映射 / 新鲜度 / 失败处理） | `docs/MD-生成-总览.md` | §3 数据获取与可靠性 |
| **质量怎么保证**（5 步校验 / 15 条跨字段规则） | `docs/MD-生成-总览.md` | §4 质量保证 |
| **任务怎么接**（输入/输出/错误码/并发幂等） | `docs/MD-生成-总览.md` + 本文件"错误码"节 | §5 调用契约 |
| **为什么这样做**（业务目标 / AI 原则 / 业务流程） | `docs/MD-生成-总览.md` §1 + `docs/AUTOCAVA 购车报告需求.md` | §1 业务与原则 |
| **边界在哪**（我们做的 / 不做的） | `docs/MD-生成-总览.md` | §6 边界与依赖 |

**反查速记**：
- 看到 `{{变量名}}` → 查 v3 模板的"变量说明"
- 看到 "这个字段是系统给还是 AI 填？" → 查 `MD-生成-总览.md` §2
- 看到 "取不到数据怎么办？" → 查 `MD-生成-总览.md` §3
- 看到 "校验报 X 错" → 查 `MD-生成-总览.md` §5 + 本文件"错误码"节
...
读取 `docs/MD-生成-总览.md` §2 字段表 作为权威，按"系统 / 混合 / AI"分类填数据：
...
- 价格 / 版本缺失 → 阻塞
- 兜底策略见 `docs/MD-生成-总览.md` §3 数据获取与可靠性
...
按 `docs/MD-生成-总览.md` §4 质量保证 5 步校验：
1. 字段完整性
2. 跨字段一致性（15 条）
...
- v3.2（2026-08-27）：新增"文档地图"节 + 修复悬空引用（v2.0 文档体系合并 4 份技术文档为 `MD-生成-总览.md` 后的同步）

---

*相关文档：*
- *技术宣讲（一站式）：`docs/MD-生成-总览.md`*
- *业务宣讲：`docs/AUTOCAVA 购车报告需求.md`*
- *AI 骨架（字段结构权威）：`assets/standard-template-v3.md`*
- *AI 样例：`examples.md`*
